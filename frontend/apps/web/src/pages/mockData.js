...
// ─── ACWR / Training Load ─────────────────────────────────
export const mockTrainingLoad = Array.from({ length: 28 }, (_, i) => {
...
function generateTrack(numPoints) {
    const baseLat = -23.55;
    const baseLon = -46.63;
...
// ─── Plan / Calendar ──────────────────────────────────────
export const mockPlan = Array.from({ length: 8 }, (_, w) => ({
    week: w + 1,
    label: `Semana ${w + 1}${w === 0 ? ' (atual)' : ''}`,
    days: [
        { day: `2026-07-${String(6 + w * 7 + 0).padStart(2, '0')}`, type: 'run', name: 'Corrida Leve', description: '8 km Z2', target_pace: '5:30/km', target_hr: '145-155', duration_min: 44, distance_km: 8 },
        { day: `2026-07-${String(6 + w * 7 + 1).padStart(2, '0')}`, type: 'strength', name: 'Força — Superior', description: 'Circuito funcional, 4 séries', duration_min: 40 },
        { day: `2026-07-${String(6 + w * 7 + 2).padStart(2, '0')}`, type: 'run', name: 'Intervalados 5x1km', description: 'Tiro a 4:30, recuperação 2min', target_pace: '4:30/km', target_hr: '172+', duration_min: 38, distance_km: 7 },
        { day: `2026-07-${String(6 + w * 7 + 3).padStart(2, '0')}`, type: 'run', name: 'Recuperação Ativa', description: '5 km leve, soltar', target_pace: '6:00/km', target_hr: '130-140', duration_min: 30, distance_km: 5 },
        { day: `2026-07-${String(6 + w * 7 + 4).padStart(2, '0')}`, type: 'hyrox', name: 'HYROX — Estações', description: 'Simulação 2 estações x3', duration_min: 45 },
        { day: `2026-07-${String(6 + w * 7 + 5).padStart(2, '0')}`, type: 'crossfit', name: 'WOD CrossFit', description: 'AMRAP 20min', duration_min: 45 },
        { day: `2026-07-${String(6 + w * 7 + 6).padStart(2, '0')}`, type: 'run', name: 'Longão', description: '16-18 km progressivo', target_pace: '5:20→4:50/km', target_hr: '150-165', duration_min: 95, distance_km: 17 },
    ].filter(Boolean),
}));
// ─── Coach Verdict ──────────────────────────────────────────
...
export const mockActivitiesList = mockActivities.map((w, i) => ({
