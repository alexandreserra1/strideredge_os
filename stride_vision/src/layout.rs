//! layout.rs — layout de keypoints (COCO-17 do YOLO; BlazePose33 do produto).
//!
...
//! (quadril/joelho/tornozelo...). Trocar o motor de pose = apontar o backend pra outro
//! `KeypointLayout`; o parsing do tensor, o desenho e as métricas seguem sem mudança porque
//! falam em NOMES, não em números mágicos. Os pontos de pé são Option: existem no BlazePose33,
//! não no COCO-17 (com YOLO a pisada é INFERIDA e não há dorsiflexão).

/// Descreve um conjunto de keypoints: contagem, nomes, esqueleto e os índices semânticos que a
/// biomecânica usa. Layouts diferentes (COCO-17, BlazePose33) são só instâncias distintas.
#[derive(Debug)]
pub struct KeypointLayout {
    ...
    pub ankle_l: usize,
    pub ankle_r: usize,
    // pontos de PÉ — Some só quando o layout os traz (BlazePose33). None = pisada inferida, sem dorsiflexão.
...
mod tests {
    ...
    #[test]
    fn layout_by_name_resolve_os_dois_e_recusa_desconhecido() {
