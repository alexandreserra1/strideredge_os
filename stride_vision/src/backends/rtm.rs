...
impl RtmPose26Backend {
    ...
    fn detector_input(img: &RgbImage) -> (Array4<f32>, f32) {
        ...
        let mut input =
            Array4::<f32>::from_elem((1, 3, RTMDET_INPUT as usize, RTMDET_INPUT as usize), 114.0);
    ...
    fn detect(&mut self, img: &RgbImage) -> Result<Option<(f32, RtmCrop)>> {
        ...
        if dets.ndim() != 3 || dets.shape()[0] != 1 || dets.shape()[2] != 5 {
            return Err(anyhow!(
                "ONNX RTMDet incompatível: esperado dets [1,N,5], recebido {:?}",
                dets.shape()
            ));
        }
    ...
    fn bgr_bilinear(img: &RgbImage, x: f32, y: f32) -> [f32; 3] {
        ...
        let mut out = [0.0; 3];
        for channel in 0..3 {
            out[channel] = a[channel] * (1.0 - dx) * (1.0 - dy)
                + b[channel] * dx * (1.0 - dy)
                + c[channel] * (1.0 - dx) * dy
                + d[channel] * dx * dy;
        }
    ...
    fn pose_input(img: &RgbImage, crop: RtmCrop) -> Array4<f32> {
        let mut input =
            Array4::<f32>::zeros((1, 3, RTMPOSE_HEIGHT as usize, RTMPOSE_WIDTH as usize));
        for y in 0..RTMPOSE_HEIGHT as usize {
            for x in 0..RTMPOSE_WIDTH as usize {
                ...
                for channel in 0..3 {
                    input[[0, channel, y, x]] =
                        (pixel[channel] - RTMPOSE_MEAN[channel]) / RTMPOSE_STD[channel];
                }
            }
        }
        input
    ...
    fn decode(&mut self, img: &RgbImage, crop: RtmCrop, confidence: f32) -> Result<Pose> {
        ...
        if outputs.len() != 2 {
            return Err(anyhow!(
                "ONNX RTMPose incompatível: esperado duas saídas SimCC"
            ));
        }
        let (simcc_x, simcc_y) = (
            o(outputs[0].try_extract_array::<f32>())?,
            o(outputs[1].try_extract_array::<f32>())?,
        );
        if simcc_x.ndim() != 3
            || simcc_y.ndim() != 3
            || simcc_x.shape()[0] != 1
            || simcc_y.shape()[0] != 1
            || simcc_x.shape()[1] != HALPE26.count
            || simcc_y.shape()[1] != HALPE26.count
        {
            return Err(anyhow!(
                "ONNX RTMPose incompatível: esperado SimCC [1,26,X]/[1,26,Y], recebido {:?}/{:?}",
                simcc_x.shape(),
                simcc_y.shape()
            ));
        }
        ...
        for k in 0..HALPE26.count {
            let x_values: Vec<f32> = (0..simcc_x.shape()[2])
                .map(|i| simcc_x[[0, k, i]])
                .collect();
            let y_values: Vec<f32> = (0..simcc_y.shape()[2])
                .map(|i| simcc_y[[0, k, i]])
                .collect();
            ...
            keypoints.push((
                x.clamp(0.0, img.width() as f32),
                y.clamp(0.0, img.height() as f32),
...
impl PoseBackend for RtmPose26Backend {
    ...
    fn infer(&mut self, img: &RgbImage, _timestamp_ms: u64) -> Result<Option<Pose>> {
        let Some((confidence, crop)) = self.detect(img)? else {
            return Ok(None);
        };
